class Archivos::CategoriaController < ApplicationController
	#protect_from_forgery except: :listar
	def listar
		rpta = nil
		status = 200
		begin
			rpta = Archivos::Categoria.all().to_a.to_json
		rescue Exception => e
			rpta = {
				:tipo_mensaje => 'error',
				:mensaje => [
			...
			begin
				if nuevos.length != 0
					nuevos.each do |nuevo|
						n = Archivos::Categoria.new(:nombre => nuevo['nombre'],)
						n.save
				...
				if editados.length != 0
					editados.each do |editado|
						e = Archivos::Categoria.where(:id => editado['id']).first
				...
				if eliminados.length != 0
					eliminados.each do |eliminado|
						Archivos::Categoria.where(:id => eliminado).delete
					end
				end
			rescue Exception => e
				Sequel::Rollback
  ...
  def nombre
    ...
    status = 200
    begin
      temp = Archivos::Categoria.select(:nombre).where(:id => params[:categoria_id]).first
      rpta = temp.nombre
    rescue Exception => e
      rpta = {
        :tipo_mensaje => 'error',
        :mensaje => [
          'Se ha producido un error en obtener el nombre de la categorias',
          e.message
        ]
      }.to_json
      status = 500
